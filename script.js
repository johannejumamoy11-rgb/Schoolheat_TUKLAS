...
function drawGauge(val) {
  ...
  const cssSize = Math.min(wrap ? wrap.clientWidth : 280, 280);
  cvs.width = cssSize * dpr; cvs.height = cssSize * dpr;
  cvs.style.width = cssSize + 'px'; cvs.style.height = cssSize + 'px';
  ctx.scale(dpr, dpr);
  const cx = cssSize / 2, cy = cssSize / 2, r = cssSize * 0.39;
...
function chartZoomIn() {
  ...
  renderForecast();
  toast('Chart zoom: ' + Math.round(state.chartScale * 100) + '%', 'inf');
}
...
function renderMap() {
  ...
  const imgWrap = $('.map-img-wrap');
  if (!box) return;

  // Ensure map markers container matches image wrapper size
  if (imgWrap) {
  ...
  // Markers with exact percentage positioning relative to image
...
function mapZoomIn() { 
  state.mapZoom = Math.min(state.mapZoom * 1.25, 4); 
  applyMapTransform(); 
  toast('Zoom: ' + Math.round(state.mapZoom * 100) + '%', 'inf');
}
function mapZoomOut() { 
  state.mapZoom = Math.max(state.mapZoom / 1.25, 0.5); 
  applyMapTransform(); 
  toast('Zoom: ' + Math.round(state.mapZoom * 100) + '%', 'inf');
}
function mapReset() { 
...
function applyMapTransform() {
  ...
  if (stage) {
    stage.style.transform = 'translate(' + state.mapPan.x + 'px, ' + state.mapPan.y + 'px) scale(' + state.mapZoom + ')';
  }
}
...
function initMapInteraction() {
  ...
  viewport.addEventListener('touchend', () => { state.mapDragging = false; viewport.style.cursor = 'grab'; });
  ...
  viewport.addEventListener('wheel', e => {
    ...
    const zoomRatio = state.mapZoom / oldZoom;
    state.mapPan.x = state.mapPan.x * zoomRatio - cx * (zoomRatio - 1);
    state.mapPan.y = state.mapPan.y * zoomRatio - cy * (zoomRatio - 1);
    applyMapTransform();
  }, { passive: false });
